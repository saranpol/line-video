...
var ProfileImage = React.createClass({displayName: "ProfileImage",
    ...
    render: function() {
        ...
        var progressStyle = {width:this.state.progress+"%"}

        var progressDiv = ('');
        if(this.state.progress > 0){
            progressDiv = ( React.createElement("div", {className: "progress"}, 
                                React.createElement("div", {className: "progress-bar", role: "progressbar", "aria-valuenow": "{this.state.percent}", "aria-valuemin": "0", "aria-valuemax": "100", style: progressStyel}, 
                                    this.state.percent, "%"
                                )
                            ))
        }
